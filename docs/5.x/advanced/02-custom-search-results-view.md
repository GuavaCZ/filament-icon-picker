...
| Name | What it is |
|------|------------|
| `resultsVisible` | The icons to render right now, an array of `{ id, label, set, custom }` |
| `state` | The ID of the currently selected icon |
| `isLoading` | Whether the icons are still being loaded |
...
Icons are not sent to the browser as SVG, only as their ID and label. To draw one, call `setElementIcon` on the element it should end up in. Doing it when the element scrolls into view keeps large sets fast, and it is what lets us request the markup of many icons in one go:
