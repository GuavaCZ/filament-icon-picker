...
So the scope is also checked when the form is submitted, and again on the endpoints the picker loads its icons from. The scope is part of the encrypted token the field issues, so a browser cannot ask for a scope it wasn't given. An icon from a different scope is rejected in both places.
